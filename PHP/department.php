...
<form name="dropdown">
 ...
 <option value="department.php">Please select one</option>
...
<br>
<div style="height:600px;width:700px;font:16px/26px Georgia, Garamond, Serif;overflow:auto;">
        <table border = "1">
        <thead>
            <tr>
                <td><center>Department Name<td></center>
                <td><center>Budget Code<td></center>
				<td><center>Office Number<td></center>
				<td><center>Department Phone<td></center>
            </tr>
        </thead>
        <tbody>
        <?php
            ...
            $results = $conn->query("SELECT * FROM department order by departmentname");
            while($row = $results->fetch_assoc()) {
            ?>
                <tr>
                    <td><center><?php echo $row['DepartmentName']?><td></center>
                    <td><center><?php echo $row['BudgetCode']?><td></center>
					<td><center><?php echo $row['OfficeNumber']?><td></center>
					<td><center><?php echo $row['DepartmentPhone']?><td></center>
                </tr>
